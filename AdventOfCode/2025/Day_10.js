...
const input = require("./input/Day_10");

String.prototype.replaceAt = function (index, replacement) {
    return this.substring(0, index) + replacement + this.substring(index + replacement.length);
}

const calculateMinimumButtonPresses = (input) => {

    const machines = input.split("\n");

    let buttonPresses = 0;

    const entireStart = new Date();

    for (const machine of machines) {
        const config = machine.match(/\[(.*)\]/)?.[1];
        const buttons = Array.from(machine.matchAll(/\((\d+(?:,\d+)*)\)/g)).map(match => match[1].split(",").map(Number));

        const start = new Date();

        buttonPresses += calculateButtonPress(config, buttons);
    ...
    const entireEnd   = new Date();
    const entireTimeInSeconds = (entireEnd.getTime() - entireStart.getTime()) / 1000;

    console.log(entireTimeInSeconds);
    console.log(buttonPresses);

}

const calculateButtonPress = (config, buttons) => {

    const totalCombinationsLength = Math.pow(2, buttons.length);

    const totalCombinations = [];

    for (let i = 0; i < totalCombinationsLength; i++) {
        const combinations = [];
        for (let j = 0; j < buttons.length; j++) {
            if ((i >> j) & 1) {
                combinations.push(buttons[j]);
            }
        }
        totalCombinations.push(combinations);
    }

    let minButtonPresses = buttons.length;

    for (const combination of totalCombinations) {
        if (combination.length >= minButtonPresses) continue;
        let initialConfig = ".".repeat(config.length);
        for (const button of combination) {
            for (const pos of button) {
                if (initialConfig[pos] === ".") {
                    initialConfig = initialConfig.replaceAt(pos, "#");
                } else {
                    initialConfig = initialConfig.replaceAt(pos, ".");
                }
            }
        ...
        if (initialConfig === config) {
            minButtonPresses = combination.length;
        }
    }
...
// calculateMinimumButtonPresses(sampleInput);

calculateMinimumButtonPresses(input);
