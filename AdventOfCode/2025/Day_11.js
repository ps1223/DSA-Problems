...
const calculateTotalRoutes = (input) => {
    ...
    findTillOut(map, 'svr', output);

    console.log(output.count);

}

const findTillOut = (map, curr, output) => {

    if (map[curr].includes('fft')) {
        output.count++;
        return;
    }

    for (const outputDevice of map[curr]) {
        findTillOut(map, outputDevice, output);
    }
