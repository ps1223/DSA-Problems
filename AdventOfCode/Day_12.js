...
const input = require("./input/Day_12")

const calculateFittingGifts = (input) => {

    const data = input.split("\n\n");

    const regions = data.splice(data.length - 1)[0].split("\n");

    const shapes = data.map(shape => shape.split(":\n").splice(1)[0]).map(shape => shape.split("\n"));
    ...
    for (const region of regions) {
        const [row, column] = region.split(": ")[0].split('x');
        const requirements = region.split(": ")[1].split(" ");
        ...
        for (let i = 0; i < requirements.length; i++) {
            ...
            requiredSize += requirement * shapeCount[i];
    ...
    /*
        Just trid to limit the input based on the size required and remove cases which are larger than the required space
        Seems to a troll on last puzzle and the answer is accepted
     */
...
// calculateFittingGifts(sampleInput);

calculateFittingGifts(input);
