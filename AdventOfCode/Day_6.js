...
const input = require("./input/Day_6");

const calculateMathProblem = (input) => {

    const lines = input.split("\n").map(line => line.split(" ").filter(val => !!val));

    const [operations] = lines.splice(- 1, 1);

    const resultArray = Array(operations.length).fill(null);
...
console.log(calculateMathProblem(input));
// console.log(calculateMathProblem(sampleInput));
