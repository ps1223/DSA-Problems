...
const calculateCircuitProduct = (input) => {
    ...
    while(count > 0) {
        ...
        for(let i = 0; i < list.length - 1; i++) {
            for( let j = i + 1; j < list.length; j++) {
                ...
                if(!checkedBoxes[`${box1},${box2}`]) {
                    const distance = findThreeDDistance(box1, box2);
        ...
        console.log(circuits.length, count);

    }

    if(circuits.length < 3) {
        for(const box of list) {
            if(circuits.findIndex(circuit => circuit.includes(box)) === -1) {
                circuits.push(box);
            }
            if(circuits.length === 3) break;
        }
    }

    const sorted = circuits.map(circuit => circuit.length).sort((a, b) => b - a);
